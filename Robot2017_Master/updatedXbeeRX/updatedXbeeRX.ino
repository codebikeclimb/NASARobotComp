...
// (optimal) -40 : -30 dBm or higher ===== Amazing **** beacon is pointing directly at robot. This heading is what we need
// 
...
int maxRSSI = readings[0].signalStrength;
...
void loop() {
  ...
  for (int i=1; i < samples; i++) {
    if (maxRSSI < readings[i].signalStrength && readings[i].signalStrength != -1000) {
